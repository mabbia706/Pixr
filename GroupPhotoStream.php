...
<script src="js/GroupProfile.js"></script>
...
$.ajax({
  type: "POST",
  url: "controller/UserController.php",
  data: { action: "updateGroupCoverPhoto", user: currentUser,photo:pictures[slideShowPool[0].currentId].getPath(),group:group_name },async:false
})
  .done(function( data ) {
  alert('Cover photo updated reload Group to see changes');
...
var comment=new Comment(this.currentId);
	document.getElementById('uploader').innerHTML=pictures[this.currentId].getUploader();
...
document.getElementById('uploader').innerHTML=pictures[slideShowPool[id].currentId].getUploader();
...
slideShowPool[id].incrementCurrentId();
	...
	document.getElementById('slideshow').appendChild(document.createElement("BR"));
	document.getElementById('slideshow').appendChild(document.createElement("BR"));
...
if(sessionStorage.getItem('currentGroupAdmin')==sessionStorage.getItem('page1content')){
...
$.ajax({
 ...
 // alert(data[i]['image_name']);
    ...
    loadPicture(data[i]['image_name'],data[i]['image_data'],data[i]['title'],data[i]['description'],data[i]['tags'],data[i]['access_rights'],data[i]['uploader'],data[i]['date_created']);
