...
document.getElementById('albumGallery').style.width=width+"px";
document.getElementById('albumGallery').style.height=height+"px";
	...
	buttons:{
		...
		function(){
			...
			$.ajax({
						  type: "POST",
						  url: "controller/UserController.php",
						  data: { action: "checkAlbumName", user: currentUser,album_title:title },async:false
						})
						  .done(function( data ) {
						  if(data.length==12){
						  duplicate=1;
				...
				$('#Error').html('Please Select at least one image to create album');
						...
						var currDate=new Date();
						 ...
						 $.ajax({
						  type: "POST",
						  url: "controller/UserController.php",
						  data: { action: "Add Pictures To Album", user: currentUser,album_title:title,pics:images,date:currDate },async:false
						})
						  .done(function( data ) {
...
$.ajax({
  type: "POST",
  url: "controller/UserController.php",
  data: { action: "getAlbums", user: currentUser },async:false
}).done(function( data ) {
  data=JSON.parse(data);
  for(i=0;i<data.length;i++){
...
$('#albumGallery').empty();
  ...
  type: "POST",
  url: "controller/UserController.php",
  data: { action: "getPicturesFromAlbum", user: currentUser,album_title:title },async:false
	}).done(function( data ) {
	temp_names=[];
	var duplicate=0;
	  data=JSON.parse(data);
	  for(i=0;i<data.length;i++){
		 ...
		 if(duplicate==0){
		  ...
		  albums[t].addPicture(data[i]['image_name'],data[i]['image_data'],data[i]['title'],data[i]['description'],data[i]['tags'],data[i]['access_rights'],data[i]['Email']);
			}
			duplicate=0;
		}
  });
